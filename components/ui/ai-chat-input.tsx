...
export const PromptInput = React.forwardRef<HTMLDivElement, PromptInputProps>(function PromptInput(
  ...
  function startRecording() {
    ...
    setIsRecording(true);
    try { recognition.start(); } catch { recognitionRef.current = null; setIsRecording(false); setVoiceError(labels.voiceDenied); }
  }
    ...
    <div ref={assignRoot} data-testid="prompt-input" className={cn("relative mx-auto w-full max-w-[780px]", className)}>
      ...
      <ChatOverlay open={settingsOpen} onClose={() => setSettingsOpen(false)} labelledBy="chat-model-picker-title" position="responsive" className="p-3" closeLabel={labels.close}>
        <div>
          ...
          {attachmentsEnabled && (
            <section className="border-b border-outline-variant px-2 py-4">
              <div className="mb-2 flex items-center justify-between gap-3"><p className="label-caps text-on-secondary-container">{locale === "ru" ? "Прикреплённые файлы" : "Attached files"}</p><button type="button" onClick={() => fileInputRef.current?.click()} disabled={attachments.length >= 3 || disabled} className="inline-flex min-h-10 items-center gap-2 rounded-xl bg-surface-container-low px-3 text-[11px] text-primary disabled:opacity-30"><Plus size={14} />{labels.addAttachment}</button></div>
