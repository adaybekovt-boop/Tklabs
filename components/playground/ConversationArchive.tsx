...
import { Check, Pencil, Pin, Search, Trash2, X } from "lucide-react";
...
export function ConversationArchive({ locale, onNavigate, headingId = "conversation-history-title", compact = false }: ConversationArchiveProps) {
  ...
  const filteredSessions = useMemo(
    () => sessions.filter((session) => `${session.title} ${session.model}`.toLocaleLowerCase(locale === "ru" ? "ru-RU" : "en-US").includes(normalizedQuery)),
  ...
  return (
    <section className={compact ? "pt-3" : "mt-8 border-t-[0.5px] border-primary pt-6"} aria-labelledby={headingId}>
        ...
        <div className="flex min-h-9 items-center gap-1">
          ...
          {sessions.length > 0 && !clearArmed && (
            <button
              ...
              className="min-h-9 rounded-full px-2 text-[10px] uppercase tracking-[0.08em] text-on-secondary-container hover:bg-surface-container-low hover:text-primary"
            >
              {ui.clear}
            </button>
          )}
          {sessions.length > 0 && clearArmed && (
            <>
              <button
                ...
                className="min-h-9 rounded-full bg-error px-3 text-[10px] uppercase tracking-[0.08em] text-on-primary"
              >
                {ui.confirmClear}
              </button>
              <button
                ...
                className="grid size-9 place-items-center rounded-full text-on-secondary-container hover:bg-surface-container-low hover:text-primary"
      ...
      {sessions.length > 0 && (
        <label className="relative mb-3 block">
          ...
          <input
            ...
            className="h-11 w-full rounded-2xl border border-outline-variant bg-surface-container-low pl-9 pr-3 text-[13px] text-primary outline-none focus:border-primary"
          />
        </label>
      )}
      ...
      {sessions.length === 0 ? (
        ...
        <div className="flex max-h-[min(52vh,420px)] flex-col gap-1 overflow-y-auto pr-1">
          {filteredSessions.map((session) => {
            ...
            return (
              <div
                key={session.id}
                className={cn(
                  "group flex min-h-14 items-center gap-1 rounded-2xl border px-2 py-1.5 transition-[border-color,background-color,transform] hover:-translate-y-px hover:border-primary",
                ...
                {editing ? (
                  <div className="flex min-w-0 flex-1 items-center gap-1">
                    <input
                      ...
                      className="h-10 min-w-0 flex-1 rounded-xl border border-primary bg-surface-container-lowest px-3 text-[13px] text-primary outline-none"
                    />
                    <button type="button" onClick={() => saveRename(session.id)} className="grid size-10 place-items-center rounded-full text-primary hover:bg-surface-container" aria-label={ui.save}>
                      <Check size={15} />
                    </button>
                    <button type="button" onClick={cancelRename} className="grid size-10 place-items-center rounded-full text-on-secondary-container hover:bg-surface-container hover:text-primary" aria-label={ui.cancel}>
                      <X size={15} />
                    </button>
                  </div>
                ) : (
                  <>
                    ...
                    <div className="flex shrink-0 items-center">
                      <button
                        ...
                        aria-label={session.pinned ? ui.unpin : ui.pin}
                        aria-pressed={session.pinned === true}
                        className={cn(
                          "grid size-9 place-items-center rounded-full text-on-secondary-container hover:bg-surface-container hover:text-primary",
                          session.pinned && "text-primary",
                        )}
                      >
                        <Pin size={14} className={session.pinned ? "fill-current" : undefined} />
                      </button>
                      <button
                        type="button"
                        onClick={() => beginRename(session)}
                        aria-label={`${ui.rename}: ${session.title}`}
                        className="grid size-9 place-items-center rounded-full text-on-secondary-container hover:bg-surface-container hover:text-primary"
                      >
                        <Pencil size={14} />
                      </button>
                      <button
                        ...
                        aria-label={`${ui.remove}: ${session.title}`}
                        className="grid size-9 place-items-center rounded-full text-on-secondary-container hover:bg-error-container hover:text-error"
                      >
                        <Trash2 size={14} />
                      </button>
                    </div>
