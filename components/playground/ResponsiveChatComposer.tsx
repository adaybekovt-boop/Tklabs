...
function ModelRow({ model, selected, onSelect }: { model: ChatInputModel; selected: boolean; onSelect: () => void }) {
  return (
    <button
      ...
      onClick={onSelect}
      className={cn(
        "my-1 flex min-h-12 w-full items-center gap-3 rounded-2xl px-3 py-2.5 text-left text-sm disabled:opacity-35",
        selected ? "bg-surface-container-low text-primary" : "text-on-surface-variant hover:bg-surface-container-low",
      )}
    >
...
const MobileChatComposer = React.forwardRef<HTMLDivElement, PromptInputProps>(function MobileChatComposer(
  ...
  async function addFiles(files: FileList | null) {
    ...
    for (const file of candidates.slice(0, room)) {
      ...
      const additionLength = Array.from(`[${cleanName}]\n${cleanContent}`).length + 2;
      if (!cleanName || Array.from(cleanName).length > 120 || !cleanContent || new TextEncoder().encode(cleanContent).byteLength > maxAttachmentBytes || contextLength + additionLength > maxAttachmentContextLength) {
        rejected = true;
        continue;
      }
      next.push({ id: `${cleanName}-${file.lastModified}-${crypto.randomUUID()}`, file, name: cleanName, content: cleanContent, url: URL.createObjectURL(file) });
    ...
    <div ref={forwardedRef} data-testid="mobile-prompt-input" className={cn("relative mx-auto w-full max-w-[780px]", className)}>
      ...
      <div className="overflow-hidden rounded-[1.55rem] border border-outline-variant bg-surface-container-lowest shadow-[0_10px_30px_color-mix(in_srgb,var(--color-primary)_8%,transparent)] focus-within:border-primary">
        <div className="flex items-center gap-2 px-3 pb-1 pt-2.5">
          <button type="button" onClick={() => setSettingsOpen(true)} className="flex min-h-9 min-w-0 items-center gap-2 rounded-full bg-surface-container-low px-3 text-left text-[11px] font-medium text-primary" aria-expanded={settingsOpen} aria-haspopup="dialog">
            <span className="truncate">{selectedModel?.name ?? labels.model}</span><ChevronDown size={13} className="shrink-0" />
          </button>
          <button type="button" onClick={() => setSettingsOpen(true)} className="min-h-9 shrink-0 rounded-full px-3 text-[11px] text-on-secondary-container hover:bg-surface-container-low">{chatResponseModeLabel(responseMode, locale)}</button>
          <button type="button" onClick={() => setSettingsOpen(true)} className="ml-auto grid size-9 shrink-0 place-items-center rounded-full text-on-secondary-container hover:bg-surface-container-low hover:text-primary" aria-label={locale === "ru" ? "Настройки ответа" : "Response settings"}><SlidersHorizontal size={16} /></button>
        </div>

        {attachments.length > 0 && (
          <div className="flex gap-2 overflow-x-auto px-3 py-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {attachments.map((attachment) => (
              <div key={attachment.id} className="relative flex h-11 max-w-[190px] shrink-0 items-center gap-2 rounded-xl border border-outline-variant bg-surface px-3 pr-9">
                <button type="button" onClick={() => setActiveAttachment(attachment)} className="flex min-w-0 items-center gap-2 text-left text-[11px] text-on-surface-variant" aria-label={`${labels.openAttachment} ${attachment.name}`}><FileText size={15} className="shrink-0" /><span className="truncate">{attachment.name}</span></button>
        ...
        <div className="flex items-end gap-1 px-2 pb-2 pt-1">
          <button
            type="button"
            onClick={() => attachmentsEnabled ? fileInputRef.current?.click() : setSettingsOpen(true)}
            disabled={disabled || (attachmentsEnabled && attachments.length >= 3)}
            ...
            aria-label={attachmentsEnabled ? labels.addAttachment : labels.model}
          >
            <Plus size={19} />
      ...
      <ChatOverlay open={settingsOpen} onClose={() => setSettingsOpen(false)} labelledBy="mobile-chat-settings-title" position="sheet" className="p-3" closeLabel={labels.close}>
        <div className="flex items-center justify-between gap-3 border-b border-outline-variant px-2 pb-3">
          <div><p className="label-caps text-on-secondary-container">AI CHAT</p><h2 id="mobile-chat-settings-title" className="mt-1 text-lg font-medium text-primary">{locale === "ru" ? "Модель и режим ответа" : "Model and response mode"}</h2></div>
          <button type="button" onClick={() => setSettingsOpen(false)} className="grid size-11 place-items-center rounded-full text-primary hover:bg-surface-container-low" aria-label={labels.close}><X size={17} /></button>
        </div>
        ...
        <section className="border-b border-outline-variant px-2 py-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            {efforts.map((entry) => <button key={entry.id} type="button" onClick={() => setEffort(entry.id)} aria-pressed={effort === entry.id} className={cn("min-h-11 rounded-xl border text-[11px]", effort === entry.id ? "border-primary bg-primary text-on-primary" : "border-outline-variant bg-surface")}>{entry.label}</button>)}
          </div>
        ...
        <div role="listbox" aria-label={labels.model} className="px-2 py-4">
          <p className="label-caps mb-2 text-on-secondary-container">{labels.model}</p>
          {groupedModels.map(([tier, tierModels]) => (
            <section key={tier} className="border-b border-outline-variant py-2 last:border-0">
              <p className="label-caps px-2 pb-2 text-on-secondary-container">{tier}</p>
              {tierModels.map((model) => <ModelRow key={model.id} model={model} selected={selectedModelId === model.id} onSelect={() => { onModelChange(model.id); setSettingsOpen(false); }} />)}
            </section>
          ))}
        </div>
