...
export function TermsGate({ enabled, locale }: { enabled: boolean; locale: TermsLanguage }) {
  ...
  return (
    <div className="fixed inset-0 z-[220] grid overflow-y-auto bg-primary/75 p-4 sm:p-8" role="dialog" aria-modal="true" aria-labelledby="terms-gate-title" data-terms-gate>
        ...
        {stage === "language" ? (
          <div className="mx-auto max-w-xl py-8 text-center sm:py-14">
            ...
            <div className="mt-10 grid gap-3 sm:grid-cols-2">
              {(["ru", "en"] as const).map((nextLanguage) => <button key={nextLanguage} type="button" className="quiet-button quiet-button--dark w-full" onClick={() => { setLanguage(nextLanguage); setStage("terms"); setError(null); }}>{getTerms(nextLanguage).languageName}</button>)}
            </div>
            {error && <div className="mt-6 space-y-3" aria-live="polite"><p className="text-[13px] text-error">{error}</p><button type="button" className="label-caps border-b border-primary pb-1 text-primary" onClick={() => { setError(null); setRetryToken((value) => value + 1); }}>{text.retry}</button></div>}
          </div>
        ) : (
          <div>
            <div className="flex flex-col gap-5 border-b border-primary pb-7 sm:flex-row sm:items-start sm:justify-between">
              <div><p className="label-caps mb-4 text-secondary">{text.label}</p><h1 id="terms-gate-title" className="headline-title">{text.title}</h1><p className="mt-3 text-[14px] text-on-surface-variant">Legal bundle · {CURRENT_LEGAL_BUNDLE_VERSION}</p></div>
            ...
            <div className="mt-5 flex flex-wrap gap-x-5 gap-y-2 text-[12px] underline underline-offset-4">
              <Link href="/legal/privacy" target="_blank">Privacy</Link><Link href="/legal/acceptable-use" target="_blank">Acceptable Use</Link><Link href="/legal/ai-transparency" target="_blank">AI Transparency</Link><Link href="/legal/subprocessors" target="_blank">Subprocessors</Link>
            </div>
            <div className="mt-8 max-h-[45vh] overflow-y-auto border-y border-outline-variant py-7 pr-3"><div className="space-y-10">{text.sections.map((section) => <section key={section.title}><h2 className="font-serif text-[23px] leading-[1.25]">{section.title}</h2><div className="mt-4 space-y-4 text-[14px] leading-[1.75] text-on-surface-variant">{section.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div></section>)}</div></div>
            <label className="mt-7 flex cursor-pointer items-start gap-3 text-[14px] leading-[1.6]"><input type="checkbox" className="mt-1 size-4 accent-primary" checked={checked} onChange={(event) => setChecked(event.target.checked)} /><span>{text.readAgreement} · Privacy / AUP / AI Transparency</span></label>
            {error && <p className="mt-4 text-[13px] text-error" aria-live="polite">{error}</p>}
            <div className="mt-7 flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between"><Link href="/api/auth/signout" className="text-[12px] text-secondary underline underline-offset-4">Decline / sign out</Link><button type="button" className="quiet-button quiet-button--dark" disabled={!checked || busy} onClick={() => void accept()}>{busy ? "…" : text.accept}</button></div>
          </div>
        )}
      </div>
    </div>
