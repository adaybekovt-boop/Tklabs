      ...
      <div className="m-auto w-full max-w-3xl border border-primary bg-surface p-6 shadow-2xl sm:p-10">
        {stage === "language" ? (
          ...
          <div>
            ...
            <div className="mt-7 flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between"><a href="/api/auth/signout" className="text-[12px] text-secondary underline underline-offset-4">Decline / sign out</a><button type="button" className="quiet-button quiet-button--dark" disabled={!checked || busy} onClick={() => void accept()}>{busy ? "…" : text.accept}</button></div>
          </div>
        )}
      </div>
    </div>
