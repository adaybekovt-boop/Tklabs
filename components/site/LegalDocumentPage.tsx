...
export function LegalDocumentPage({ doc }: { doc: LegalDocument }) {
  ...
  return (
    <main className="legal-page">
      ...
      <article className="legal-document"><span className="eyebrow text-[color:var(--primary)]">{copy.legal.label} / {doc}</span><h1>{document.title}</h1><p className="legal-intro">{document.intro}</p>{document.sections.map(([heading, body]) => <section key={heading}><h2>{heading}</h2><p>{body}</p></section>)}</article>
      <footer className="legal-footer"><Link href="/">{copy.common.returnToTheatre}</Link></footer>
    </main>
  );
}
