...
export type ErmaVerificationResult = { status: ErmaVerificationStatus; code: string; successfulTools: number; evidenceCount: number; independentWebSources: number; openedWebSources: number };
function successful(traces: readonly AiToolCallTrace[]) { return traces.filter((trace) => trace.status === "success"); }
function webHosts(evidence: readonly ErmaEvidence[], openedOnly = false) { const hosts = new Set<string>(); for (const entry of evidence) { if (entry.kind !== "web" || !entry.href || (openedOnly && entry.tool !== "open_web_result")) continue; try { hosts.add(new URL(entry.href).hostname.toLowerCase()); } catch { /* ignore malformed evidence */ } } return hosts; }
...
export function verifyErmaEvidence(route: ErmaIntelligenceRoute, traces: readonly AiToolCallTrace[], evidence: readonly ErmaEvidence[]): ErmaVerificationResult {
  const tools = successful(traces); const hosts = webHosts(evidence); const openedHosts = webHosts(evidence, true); const base = { successfulTools: tools.length, evidenceCount: evidence.length, independentWebSources: hosts.size, openedWebSources: openedHosts.size };
  ...
  if (route.intent === "research") { if (openedHosts.size >= 2) return { status: "verified", code: "MULTI_SOURCE_WEB_VERIFIED", ...base }; if (openedHosts.size === 1 || hosts.size >= 1) return { status: "partial", code: "RESEARCH_NEEDS_MORE_OPENED_SOURCES", ...base }; return { status: "unverified", code: "RESEARCH_EVIDENCE_REQUIRED", ...base }; }
  if (route.intent === "fact_lookup") { if (openedHosts.size >= 2) return { status: "verified", code: "FACT_MULTI_SOURCE_VERIFIED", ...base }; if (openedHosts.size === 1) return { status: "verified", code: "FACT_SOURCE_VERIFIED", ...base }; if (hosts.size >= 1) return { status: "partial", code: "FACT_SOURCE_FOUND_NOT_OPENED", ...base }; return { status: "unverified", code: "FACT_EVIDENCE_REQUIRED", ...base }; }
  if (route.freshness === "web-current") { if (openedHosts.size >= 1) return { status: "verified", code: "CURRENT_WEB_SOURCE_VERIFIED", ...base }; if (hosts.size >= 1) return { status: "partial", code: "CURRENT_SOURCE_FOUND_NOT_OPENED", ...base }; return { status: "unverified", code: "CURRENT_SOURCE_REQUIRED", ...base }; }
...
export function verificationPromptBlock(result: ErmaVerificationResult) {
  return ["VERIFICATION STATUS:", `- status: ${result.status}`, `- code: ${result.code}`, `- evidence count: ${result.evidenceCount}`, `- independent web sources found: ${result.independentWebSources}`, `- web sources opened: ${result.openedWebSources}`, result.status === "verified" ? "Use verified evidence for the claims it actually supports; verification is scoped, not a blanket guarantee." : "Do not present unverified current/source-dependent/tool-dependent claims as certain. State the limitation succinctly."].join("\n");
}
