...
export function buildAnswerDirective(route: ErmaIntelligenceRoute, verification: ErmaVerificationResult, language: "ru" | "en") {
  ...
  const intent: Record<ErmaIntelligenceRoute["intent"], string[]> = {
    ...
    fact_lookup: [
      ...
      "- If verification is partial or unverified, say that external verification was incomplete instead of presenting unsupported claims as certain.",
      "- Do not ask the user to provide sources merely because web retrieval failed. For non-current, non-legal, non-medical, non-financial, non-safety-critical and non-official-statistical questions, provide concise best-effort background from general model knowledge when useful, clearly marking it as not externally verified.",
      "- If the wording itself is genuinely ambiguous, explain the ambiguity in one or two sentences and ask one focused clarification. Do not turn a search outage into a long refusal.",
    ],
