...
export type ErmaIntelligenceRoute = {
  schemaVersion: 1;
  intent: ErmaTaskIntent;
  freshness: ErmaFreshness;
  verification: ErmaVerification;
  toolClass: ErmaToolClass;
  shouldPlanTools: boolean;
...
const CURRENT_WORDS = /(?:сегодня|сейчас|текущ\w*|последн\w*|свеж\w*|недавн\w*|новост\w*|на данный момент|today|current(?:ly)?|latest|recent|news|right now|this week|this month)/i;
const WEB_FACT_WORDS = /(?:интернет|веб|web|online|источник|source|новост|рынок|цена|стоимост|курс|погод|результат матч|выбор|президент|премьер|компан\w* сейчас|latest|today|current|news|market|price|weather|election|president|prime minister)/i;
const TKLAB_WORDS = /(?:tk\s*lab|tklab|tk labs|erma)/i;
const POLICY_WORDS = /(?:terms|agreement|privacy|политик|соглашен|конфиденц|acceptable use|aup|security|безопасност|retention|хранен|удален|provider|провайдер|данн\w*)/i;
const RELEASE_WORDS = /(?:patch|release|version|changelog|update|патч|релиз|верси|обновлен|что нового|изменил)/i;
const DOCUMENT_WORDS = /(?:pdf|документ|файл|вложен|attachment|document|contract|договор|таблиц|csv)/i;
...
function routeFor(intent: ErmaTaskIntent, prompt: string): ErmaIntelligenceRoute {
  const current = CURRENT_WORDS.test(prompt);
  const comparison = COMPARE_WORDS.test(prompt);

  if (intent === "tklab_policy") return {
    schemaVersion: 1,
    intent,
    freshness: "internal-current",
    verification: "verify-sources",
    toolClass: "internal",
    shouldPlanTools: true,
    shouldCiteSources: true,
    maxToolCalls: comparison ? 4 : 2,
    maxToolRounds: 2,
    reasonCode: "TKLAB_POLICY_SOURCE_OF_TRUTH",
  };

  if (intent === "tklab_release") return {
    schemaVersion: 1,
    intent,
    freshness: "internal-current",
    verification: "verify-sources",
    ...
    shouldPlanTools: true,
    shouldCiteSources: true,
    maxToolCalls: comparison ? 5 : 2,
    maxToolRounds: 2,
    reasonCode: "TKLAB_RELEASE_SOURCE_OF_TRUTH",
  };

  if (intent === "math") return {
    schemaVersion: 1,
    intent,
    freshness: "none",
    ...
    maxToolCalls: 4,
    maxToolRounds: 2,
    reasonCode: "DETERMINISTIC_MATH_PREFERRED",
  };

  if (intent === "fresh_information" || intent === "research") return {
    schemaVersion: 1,
    intent,
    freshness: "web-current",
    verification: "verify-sources",
    toolClass: "web",
    shouldPlanTools: true,
    shouldCiteSources: true,
    maxToolCalls: intent === "research" ? 8 : 5,
    maxToolRounds: intent === "research" ? 3 : 2,
    reasonCode: intent === "research" ? "MULTI_SOURCE_RESEARCH" : "CURRENT_EXTERNAL_FACTS_REQUIRED",
  };
  ...
  if (intent === "document") return {
    ...
    shouldPlanTools: true,
    shouldCiteSources: true,
    maxToolCalls: comparison ? 6 : 4,
    maxToolRounds: 2,
    reasonCode: "DOCUMENT_EVIDENCE_REQUIRED",
  };

  if (intent === "code") return {
    schemaVersion: 1,
    intent,
    freshness: current ? "web-current" : "none",
    verification: "verify-tools",
    toolClass: current ? "web" : "code",
    shouldPlanTools: current,
    shouldCiteSources: current,
    maxToolCalls: current ? 5 : 0,
    maxToolRounds: current ? 2 : 0,
    reasonCode: current ? "CURRENT_TECHNICAL_FACTS_REQUIRED" : "CODE_REASONING",
  };
  ...
  if (intent === "comparison") return {
    ...
    intent,
    freshness: current ? "web-current" : "none",
    verification: current ? "verify-sources" : "normal",
    toolClass: current ? "web" : "none",
    shouldPlanTools: current,
    shouldCiteSources: current,
    maxToolCalls: current ? 6 : 0,
    maxToolRounds: current ? 2 : 0,
    reasonCode: current ? "CURRENT_COMPARISON" : "STATIC_COMPARISON",
  };
  ...
  return {
    ...
    intent,
    freshness: "none",
    verification: intent === "analysis" || intent === "planning" ? "verify-tools" : "normal",
    toolClass: "none",
    shouldPlanTools: false,
    shouldCiteSources: false,
    maxToolCalls: 0,
    maxToolRounds: 0,
    reasonCode: intent === "conversation" ? "DIRECT_CONVERSATION" : "MODEL_REASONING",
  };
}
...
export function boundRouteToToolCapability(route: ErmaIntelligenceRoute, capabilityMaxCalls: number, capabilityMaxRounds: number) {
  return {
    ...route,
    maxToolCalls: clamp(route.maxToolCalls, 0, Math.max(0, capabilityMaxCalls)),
    maxToolRounds: clamp(route.maxToolRounds, 0, Math.max(0, capabilityMaxRounds)),
  };
}
