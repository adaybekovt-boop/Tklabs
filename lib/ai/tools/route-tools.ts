...
const SEARCH_FOLLOW_UP = /^(?:ну\s+|тогда\s+|а\s+|и\s+)?(?:поищ(?:и|ите)|загугл(?:и|ите)?|проверь(?:те)?\s+(?:в\s+)?(?:интернете|сети)|search(?:\s+(?:the\s+)?)?(?:web|internet)|look\s+(?:it|that|this)\s+up|google\s+(?:it|that|this))\s*[?!.]*$/iu;
...
export function isContextDependentGroundingTurn(prompt: string, originalMessageCount: number) {
  ...
  return EXPLICIT_CONTEXT_REFERENCE.test(normalized) || DEICTIC_FOLLOW_UP.test(normalized) || SEARCH_FOLLOW_UP.test(normalized);
}
