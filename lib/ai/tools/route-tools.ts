...
export type ToolAugmentation = {
  summary?: string;
  traces: AiToolCallTrace[];
};

function combineSummary(summary: string | undefined, toolContext: string | undefined) {
  return [summary, toolContext].filter(Boolean).join("\n\n").slice(0, 56_000) || undefined;
}

export async function prepareReadOnlyToolAugmentation(input: {
  request: Request;
  requestId: string;
  ...
  try {
    const result = await runNvidiaToolLoop({
      ...
      localArchive: sanitizeLocalArchiveIndex(input.localArchive),
      signal: input.signal,
      getServiceStatus: async (signal) => {
        const statusUrl = new URL("/api/status", input.request.url);
    ...
    return { summary: combineSummary(input.context.summary, result.contextBlock), traces: result.traces };
  } catch (error) {
    if (input.signal?.aborted) throw error;
    console.info("ai.tool_loop", {
      requestId: input.requestId,
      status: "skipped_after_error",
      reason: error instanceof Error ? error.message.slice(0, 80) : "unknown",
    });
    return { summary: input.context.summary, traces: [] };
  }
}
