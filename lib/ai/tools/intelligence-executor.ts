...
type IntelligenceExecutionContext = ToolExecutionContext & { documents?: ChatAttachment[]; allowCodeSandbox?: boolean };

class IntelligenceToolInputError extends Error {}

function argsObject(call: RawNvidiaToolCall) {
  const raw = call.function?.arguments;
  if (typeof raw !== "string" || raw.length > 32_000) throw new IntelligenceToolInputError("invalid_tool_arguments");
  let parsed: unknown;
...
async function withTimeout<T>(operation: (signal: AbortSignal) => Promise<T>) { const controller = new AbortController(); const timer = setTimeout(() => controller.abort("tool_timeout"), AI_TOOL_TIMEOUT_MS); try { return await operation(controller.signal); } finally { clearTimeout(timer); } }
...
export async function executeIntelligenceTool(call: RawNvidiaToolCall, context: IntelligenceExecutionContext, webSession: WebSearchSession): Promise<ToolExecutionResult> {
  ...
  try {
    ...
    if (name === "run_code_sandbox") {
      ...
      const result = await runCodeSandbox({ language: args.language, code: args.code, stdin: optionalText(args.stdin, "stdin", 8_000) });
    ...
    if (name === "search_documents") {
      ...
      const results = searchRequestDocuments(context.documents ?? [], query, limit);
    ...
    if (name === "search_tklab_knowledge") {
      ...
      const query = requiredText(args.query, "query", 300); const kinds = knowledgeKinds(args.kinds); const limit = boundedInteger(args.limit, 5, 1, 8); const results = searchTkLabKnowledge(context.language, query, kinds, limit);
    ...
    if (name === "search_web") {
      rejectUnknown(args, ["query", "count"]); const query = requiredText(args.query, "query", 400); const count = boundedInteger(args.count, 5, 1, 8); const results = await withTimeout((signal) => searchWeb(query, context.language, count, signal, webSession));
    ...
    rejectUnknown(args, ["resultId"]); const resultId = requiredText(args.resultId, "resultId", 120); const opened = await withTimeout((signal) => openWebResult(resultId, signal, webSession));
