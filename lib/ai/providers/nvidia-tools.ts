...
import { kazakhstanGroundingContext } from "@/lib/ai/intelligence/grounding";
...
function plannerSystemPrompt(language: Language, route: ErmaIntelligenceRoute, documentNames: readonly string[], prompt: string, summary?: string) {
...
function plannerMessages(input: NvidiaToolLoopInput, route: ErmaIntelligenceRoute): NvidiaPlannerMessage[] { return [{ role: "system", content: plannerSystemPrompt(input.language, route, input.documents.map((document) => document.name), input.prompt, input.summary) }, ...input.messages.slice(-4).map((message) => ({ role: message.role, content: message.content } as NvidiaPlannerMessage))]; }
