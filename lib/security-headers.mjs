const PRODUCTION_ONLY_POLICY = process.env.NODE_ENV === "production"
...
// `upgrade-insecure-requests` is production-only: applying it to the HTTP Playwright
// harness would rewrite local module/CSS requests to https://127.0.0.1 and prevent
// browser hydration even though production itself is HTTPS-only.
