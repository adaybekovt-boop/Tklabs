...
export function safetyRefusal(language: SafetyLanguage, reason: SafetyReason = "code_generation") { if (reason === "instruction_override") return language === "ru" ? "Запрос пытается изменить внутренние правила. Запрос отклонён." : "The request tries to change internal rules. The request was declined."; return language === "ru" ? "Я не помогаю с вредоносным кодом, обходом защиты или кражей данных. Запрос отклонён." : "I cannot help with harmful code, bypassing security, or stealing data. The request was declined."; }
