...
const ru: Record<LegalDocumentSlug, LegalDocument> = {
  ...
  subprocessors: {
    ...
    sections: [
      ...
      { title: "Google / Auth.js", paragraphs: ["Google OAuth используется как identity provider для поддерживаемого входа. Auth.js управляет прикладной сессией TK LAB."] },
...
const en: Record<LegalDocumentSlug, LegalDocument> = {
  privacy: { ...ru.privacy, title: "Privacy Notice", label: "Trust Center", intro: "What TK LAB receives, where data is held, and which controls are available to the user.", sections: [
    ...
    { title: "5. AI requests and files", paragraphs: ["Prompts, selected attachments, and required context are sent to the selected model provider to perform the requested feature. Do not send passwords, keys, payment data, or third-party secrets without a lawful basis.", "Local ingest accepts TXT, Markdown, CSV, JSON, and PDFs with bounded best-effort text extraction. Before sending, a file becomes part of model context."] },
  ...
  subprocessors: { ...ru.subprocessors, title: "Subprocessors & Providers", label: "Data Processing", intro: "External service categories that can participate in TK LAB operation.", sections: [
    ...
    { title: "Google / Auth.js", paragraphs: ["Google OAuth is a supported identity provider. Auth.js manages the TK LAB application session."] },
