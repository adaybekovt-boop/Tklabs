...
const PREVIEW_RELEASE: Record<Locale, PreviewReleaseNote> = {
  ru: {
    ...
    summary: "HOTFIX ERMA · ПРЕДВАРИТЕЛЬНАЯ ВЕРСИЯ. v0.24.1 исправляет production readiness для Google/Brave web search, исключает повторный failed-search раунд и делает fallback статических фактов полезнее без ослабления evidence-требований для чувствительных и текущих данных.",
    changes: [
      "Production preflight теперь требует хотя бы один полный web-search provider, уже настроенный как encrypted Cloudflare Worker secret; deployment workflow не изменяет secret state сам.",
      "Поддерживаются Gemini Google Search grounding, Brave Search и legacy Google Custom Search pair; отсутствие всех провайдеров останавливает production deploy до публикации заведомо неработающего grounded-search path.",
      "Provider API keys остаются Cloudflare secrets: они не передаются браузеру, не публикуются через --var и не записываются в репозиторий.",
      ...
      "Regression suite воспроизводит production search readiness, Cloudflare secret ownership, полезный static fallback и остановку duplicate failed-search attempts.",
    ...
    migrationNotes: [
      ...
      "Перед production deploy необходимо настроить хотя бы один search credential непосредственно как Cloudflare Worker secret; deployment workflow только проверяет наличие provider и не меняет секреты.",
  ...
  en: {
    ...
    version: CURRENT_RELEASE_VERSION,
    title: "ERMA GROUNDING RELIABILITY — Production Search Hotfix",
    summary: "ERMA HOTFIX · PRE-RELEASE. v0.24.1 fixes production readiness for Google/Brave web search, removes duplicate failed-search rounds, and makes static-fact fallback more useful without weakening evidence requirements for sensitive or current data.",
    changes: [
      "Production preflight now requires at least one complete web-search provider already configured as an encrypted Cloudflare Worker secret; the deployment workflow does not mutate secret state itself.",
      "Gemini Google Search grounding, Brave Search, and the legacy Google Custom Search pair are supported; having no provider stops production deployment before a known-broken grounded-search path is published.",
      "Provider API keys remain Cloudflare secrets: they are never sent to the browser, published through --var, or stored in the repository.",
      ...
      "The regression suite reproduces production search readiness, Cloudflare secret ownership, useful static fallback, and duplicate failed-search prevention.",
    ...
    migrationNotes: [
      ...
      "Before production deploy, configure at least one search credential directly as a Cloudflare Worker secret; the deployment workflow only validates provider presence and does not mutate secrets.",
