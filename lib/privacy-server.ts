...
export class PrivacyDataUnavailableError extends Error {
  constructor() { super("Privacy data service is unavailable."); this.name = "PrivacyDataUnavailableError"; }
}

export async function exportAccountData(emailValue: string) {
  const email = normalizeEmail(emailValue);
  if (!email) throw new PrivacyDataUnavailableError();
  try {
    const db = getDb();
    ...
    return {
      ...
      account: { email: user.email, name: user.name, image: user.image, language: user.language, createdAt: user.createdAt.toISOString(), updatedAt: user.updatedAt.toISOString() },
      legalAcceptances: acceptances.map((entry) => ({ bundleVersion: entry.bundleVersion, bundleDigest: entry.bundleDigest, language: entry.language, acceptedAt: entry.acceptedAt.toISOString() })),
      workspaceSync,
    };
  } catch (error) {
    console.error("Unable to export account privacy data", error);
...
export async function deleteAccountData(emailValue: string) {
  ...
  try {
    ...
    let accessRevocationAttempted = false;
    try { await revokeClodexAccess(email); accessRevocationAttempted = true; } catch { accessRevocationAttempted = true; }
    ...
    console.error("Unable to delete account privacy data", error);
    throw new PrivacyDataUnavailableError();
  }
}
