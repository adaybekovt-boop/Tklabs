...
test("model cards disclose provider and data handling for every public Erma tier", () => { const cards = getModelCards("en"); assert.deepEqual(cards.map((card) => card.name), ["Erma Celer", "Erma Nova", "Erma Optima"]); for (const card of cards) { assert.match(card.providerClass, /External/); assert.ok(card.dataHandling.length > 20); } });
