...
import { classifyPromptSafety, safetyRefusal } from "../lib/ai-safety.ts";
...
test("instruction override refusals preserve Erma's fixed role response", () => { assert.equal(safetyRefusal("ru", "instruction_override"), "Я не могу выйти из роли, потому что меня таким создали"); assert.equal(safetyRefusal("en", "instruction_override"), "Я не могу выйти из роли, потому что меня таким создали"); });
