...
test("v0.17.5 Release Safety remains archived after later releases", async () => { const release = await read("lib/release-version.ts"); const releaseDoc = await read("docs/releases/v0.17.5.md"); const packageJson = JSON.parse(await read("package.json")); assert.notEqual(packageJson.version, "0.17.5"); assert.match(release, /CURRENT_RELEASE_VERSION = "v0\.\d+\.\d+"/); assert.match(releaseDoc, /v0\.17\.5 — Release Safety/); assert.match(releaseDoc, /single deployment writer|migration/i); });
