...
export class DemoStreamSession {
  ...
  response() {
    ...
    return new Response(responseStream, {
      status: 200,
      headers: aiStreamHeaders(this.input.requestId, this.input.rateLimitCookie),
    });
  }

  private readonly abortProvider = () => {
    if (!this.providerController.signal.aborted) {
      this.providerController.abort(this.input.request.signal.reason);
    }
  };

  private detach() {
    this.input.request.signal.removeEventListener("abort", this.abortProvider);
  }

  private send(event: StreamEvent, payload: unknown = {}) {
    if (this.streamClosed || !this.controller) return false;
    try {
      this.controller.enqueue(encodeAiStreamEvent(event, payload));
      return true;
    } catch {
  ...
  private startPayload() {
    const { context, requestId } = this.input;
    return {
      requestId,
      status: "connecting",
      context: {
  ...
  private async handleFailure(error: unknown) {
    ...
    const aborted = this.providerController.signal.aborted
      || (error instanceof DOMException && error.name === "AbortError");

    if (aborted) {
      if (this.partialAnswer) await quota.commit();
      else await quota.release();
      if (this.partialAnswer) {
        const stoppedResult = withContextMetadata(withToolCalls({
          answer: this.partialAnswer,
          provider: "nvidia",
          actualModel: model.nvidiaModel ?? model.name,
          outputTokens: estimateTextTokens(this.partialAnswer),
          timeToFirstTokenMs: this.firstTokenAt ? this.firstTokenAt - startedAt : undefined,
      ...
      this.send("done", { requestId, stopped: true, partial: Boolean(this.partialAnswer) });
      this.close();
      return;
    }
    ...
    if (this.partialAnswer) {
      ...
      logAiRequest(meta);
      this.send("meta", meta);
      this.send("error", { error: streamInterruptedText(language), requestId, partial: true });
      this.send("done", { requestId, stopped: false, partial: true });
      this.close();
      return;
    }
    ...
    if (reason === "safety_output_blocked") {
      ...
      logAiRequest(meta);
      this.send("delta", { text: safetyResult.answer });
      this.send("meta", meta);
      this.send("done", { requestId, stopped: false });
      this.close();
      return;
    ...
    const fallback = withContextMetadata(withToolCalls(await resolveFallback({
      prompt: contextualFallbackPrompt(context, this.augmentedSummary),
      language,
      allowCode: privilegedAccount,
      requestId,
      requestedModel,
    ...
    logAiRequest(meta);
    this.send("delta", { text: fallback.answer });
    this.send("meta", meta);
    this.send("done", { requestId, stopped: false });
    this.close();
  }
