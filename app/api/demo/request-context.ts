...
export type DemoRequestPreparation =
  | { response: Response }
  | { prepared: PreparedDemoRequest };
...
export async function prepareDemoRequest(request: Request): Promise<DemoRequestPreparation> {
  ...
  if (!isTrustedRequestOrigin(request)) {
    return { response: jsonResponse({ error: "Request origin is not allowed.", requestId }, requestId, 403) };
  }
  ...
  try {
    ...
    if (error instanceof RequestBodyTooLargeError) {
      return { response: jsonResponse({ error: "Request body is too large.", requestId }, requestId, 413) };
    }
    throw error;
  }
  ...
  const model = getErmaModel(typeof normalizedBody.model === "string" ? normalizedBody.model : undefined);
  ...
  let validatedPrompt;
  try {
    validatedPrompt = validateAndBuildProviderPrompt(normalizedBody.prompt, normalizedBody.attachments, {
      privileged: privilegedAccount,
    });
  } catch (error) {
    if (error instanceof PromptValidationError) {
      return { response: promptErrorResponse(error, language, privilegedAccount, requestId) };
    }
    throw error;
  }
  ...
  try {
    ...
    if (error instanceof ChatContextValidationError) {
      return { response: contextErrorResponse(error, language, requestId) };
    }
    throw error;
  }
  ...
  if (safetyDecision.blocked) {
    return {
      response: jsonResponse(
        { error: safetyRefusal(language, safetyDecision.reason), requestId },
        requestId,
        403,
      ),
    };
