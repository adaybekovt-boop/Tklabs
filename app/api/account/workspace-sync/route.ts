...
async function userEmail() { try { return (await auth())?.user?.email?.trim().toLowerCase() ?? ""; } catch { return ""; } }
function unavailable() { return Response.json({ available: false, error: "workspace_sync_unavailable" }, noStore({ status: 503 })); }

export async function GET() {
  const email = await userEmail();
  if (!email) return Response.json({ error: "Authentication required." }, noStore({ status: 401 }));
  try { return Response.json({ available: true, snapshot: await getWorkspaceSnapshot(email) }, noStore()); }
  catch (error) { if (error instanceof WorkspaceSyncUnavailableError) return unavailable(); throw error; }
}
...
export async function DELETE(request: Request) {
  ...
  const email = await userEmail();
  if (!email) return Response.json({ error: "Authentication required." }, noStore({ status: 401 }));
  try { return Response.json({ available: true, ...(await deleteWorkspaceSnapshot(email)) }, noStore()); }
  catch (error) { if (error instanceof WorkspaceSyncUnavailableError) return unavailable(); throw error; }
}
