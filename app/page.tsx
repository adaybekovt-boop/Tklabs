  ...
  return (
    <>
      ...
      <main>
        ...
        <section className="stitch-container mb-section-gap">
          <ScrollReveal>
            <article className="relative overflow-hidden rounded-3xl border border-outline-variant bg-surface-container-lowest p-7 sm:p-10 md:p-14">
              ...
              <div className="relative grid gap-10 md:grid-cols-12 md:items-start">
                <div className="md:col-span-3">
                  <span className="inline-flex size-12 items-center justify-center rounded-2xl border border-outline-variant bg-surface-container-low">
                    <Sparkles size={20} aria-hidden="true" />
                  </span>
                  <p className="label-caps mt-6 text-secondary">{releaseLabel}</p>
                  <p className="mt-3 font-serif text-[28px]">{latestRelease.version}</p>
                  <p className="mt-2 text-sm text-secondary">{latestRelease.date}</p>
                </div>
                <div className="md:col-span-8 md:col-start-5">
                  <h2 className="headline-title">{latestRelease.title}</h2>
                  <p className="mt-5 max-w-3xl leading-[1.75] text-on-surface-variant">{latestRelease.summary}</p>
                  <ul className="mt-7 grid gap-3 sm:grid-cols-2">
                    {latestRelease.changes.slice(0, 4).map((change, index) => (
                      <li key={change} className="rounded-2xl border border-outline-variant bg-surface-container-low p-4 text-sm leading-[1.6] text-on-surface-variant">
                  ...
                  <Link href="/patch-notes" className="mt-8 inline-flex min-h-11 items-center gap-2 border-b border-primary pb-1 font-medium text-primary">
