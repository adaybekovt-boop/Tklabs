...
export default async function PatchNotesPage() {
  ...
  const entries = [
    previewRelease,
    ...evolution.filter((entry) => entry.version !== previewRelease.version),
    ...history.filter((entry) => entry.version !== previewRelease.version && !evolution.some((release) => release.version === entry.version)),
  ];
      ...
      <main className="stitch-container pb-section-gap pt-6 sm:pt-10">
        <section className="mb-8 overflow-hidden rounded-3xl border border-primary/25 bg-surface-container-lowest p-6 shadow-sm sm:mb-10 sm:p-8" data-major-preview-release>
          <div className="flex flex-wrap items-center justify-between gap-3">
            <span className="rounded-full border border-primary/30 bg-primary/10 px-3 py-1 text-[11px] font-bold uppercase tracking-[0.14em] text-primary">{ui.major}</span>
          ...
          <div className="mt-6 grid gap-3 border-t border-outline-variant/50 pt-5 md:grid-cols-2" data-preview-release-notices>
            <section className="rounded-2xl border border-outline-variant/70 bg-surface-container-low p-4" aria-labelledby="preview-known-issues-title">
              ...
              <ul className="mt-3 space-y-2 text-xs leading-5 text-on-surface-variant sm:text-[13px]">{previewRelease.knownIssues.map((item) => <li key={item}>— {item}</li>)}</ul>
            </section>
            <section className="rounded-2xl border border-outline-variant/70 bg-surface-container-low p-4" aria-labelledby="preview-migration-title">
              ...
              <ul className="mt-3 space-y-2 text-xs leading-5 text-on-surface-variant sm:text-[13px]">{previewRelease.migrationNotes.map((item) => <li key={item}>— {item}</li>)}</ul>
            </section>
          </div>
          <div className="mt-5 flex flex-col gap-2 rounded-2xl border border-outline-variant bg-surface px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <a href="https://github.com/adaybekovt-boop/Tklabs/releases" target="_blank" rel="noreferrer" className="shrink-0 text-xs font-semibold text-primary underline decoration-outline-variant underline-offset-4 hover:decoration-primary">{ui.openAutomated}</a>
          </div>
        </section>
        ...
        <section className="mb-6 flex flex-wrap items-center justify-between gap-3 border-b border-outline-variant pb-4" aria-labelledby="patch-notes-list-title">
          <div><p className="label-caps mb-1 text-secondary">{text.patchNotes.eyebrow}</p><h2 id="patch-notes-list-title" className="headline-title">{text.patchNotes.historyTitle}</h2></div>
        ...
        <section aria-label={text.patchNotes.workspaceTitle}>
          ...
          <div className="hidden lg:block"><PatchNotesBrowser entries={entries} locale={locale} /></div>
        </section>
      </main>
      <StitchFooter />
    </>
