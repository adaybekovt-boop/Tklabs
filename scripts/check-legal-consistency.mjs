...
const [legal, consent, route, schema, migration] = await Promise.all([readFile("lib/legal-documents.ts", "utf8"), readFile("lib/terms-consent.ts", "utf8"), readFile("app/api/account/terms/route.ts", "utf8"), readFile("db/schema.ts", "utf8"), readFile("drizzle/0002_legal_acceptances.sql", "utf8")]);
const failures = [];
if (!/CURRENT_LEGAL_BUNDLE_DIGEST\s*=\s*\"[a-f0-9]{64}\"/.test(legal)) failures.push("Legal bundle digest must be a 64-char hex digest");
